    ...
    public function getImageUrlAttribute()
    {
        try {
            ...
            if (preg_match('/^[a-zA-Z]:\\\\/', $image) || strpos($image, '/') === 0) {
                Log::warning('AnimalType: Rejected absolute path', ['image' => $image, 'id' => $this->id ?? null]);
                return null;
            }
            
            // Check if file exists in storage
            if (!Storage::disk('public')->exists($image)) {
                Log::warning('AnimalType: Image file does not exist', ['image' => $image, 'id' => $this->id ?? null]);
                return null;
            }
            
            // Generate relative URL to work with any domain/port
            $url = '/storage/' . $image;
            Log::info('AnimalType: Generated image URL', ['image' => $image, 'url' => $url, 'id' => $this->id ?? null]);
            return $url;
        } catch (\Throwable $e) {
            Log::error('AnimalType image URL error: ' . $e->getMessage(), [
                'image' => $this->attributes['image'] ?? null,
                'animal_type_id' => $this->id ?? null,
