...
class UpdateEpisodeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->input('episode_type') === 'video') {
            ...
            // For update, check if existing episode has audio_file or external links
            $episodeId = $this->route('episode');
            $hasExistingAudio = false;
            if ($episodeId) {
                $episode = \App\Models\PodcastEpisode::find($episodeId);
                $hasExistingAudio = $episode && ($episode->audio_file || $episode->spotify_url || $episode->apple_podcasts_url);
            }
            ...
            $rules['audio_file'] = [
                ...
                'mimes:mp3,wav',
                'max:51200', // 50MB max
                Rule::requiredIf(function () use ($hasExistingAudio) {
                    return !$hasExistingAudio && empty($this->input('spotify_url')) && empty($this->input('apple_podcasts_url'));
                }),
            ];
        }
